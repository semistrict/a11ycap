...
export function setupLibraryRoutes(app) {
    ...
    app.get("/a11ycap.js", async (req, res) => {
        try {
            // Read the browser bundle (IIFE format)
            const browserBundlePath = path.resolve(__dirname, "../../../a11ycap/dist/browser.js");
...
if (typeof window !== 'undefined' && window.A11yCap) {
  ...
  if (window.__a11ycap_mcp_connected) {
    ...
    app.get("/a11ycap.min.js", async (_req, res) => {
        // Redirect to main endpoint
        res.redirect("/a11ycap.js");
    });
    // Serve library info
    app.get("/library-info", (_req, res) => {
        res.json({
            name: "a11ycap",
            version: "1.0.0",
