...
export function setupA11yCapTools(server) {
    ...
    server.tool("list_tabs", "List all connected browser tabs with their URLs and titles", {}, async () => {
    ...
    server.tool("take_snapshot", "Take an accessibility snapshot from a connected browser", {
        browserId: z
            .string()
            .optional()
    ...
    }, async ({ browserId, mode, enableReact, ref, max_bytes, }) => {
        try {
            const connections = browserConnectionManager.getConnections();
            if (connections.length === 0) {
                return {
            ...
            const result = await browserConnectionManager.sendCommand(targetConnection.id, {
                type: "take_snapshot",
    ...
    server.tool("execute_js", "Execute JavaScript code in a connected browser", {
        browserId: z
            .string()
            .optional()
            .describe("Browser connection ID (uses first available if not specified)"),
    ...
    }, async ({ browserId, description, code, returnValue, }) => {
        try {
            const connections = browserConnectionManager.getConnections();
            if (connections.length === 0) {
                return {
    ...
    server.tool("click_element", "Click an element using its accessibility snapshot reference", {
        browserId: z
            .string()
            .optional()
            .describe("Browser connection ID (uses first available if not specified)"),
    ...
    }, async ({ browserId, element, ref, captureSnapshot, }) => {
        try {
            const connections = browserConnectionManager.getConnections();
            if (connections.length === 0) {
                return {
            ...
            if (captureSnapshot) {
                const snapshot = await browserConnectionManager.sendCommand(targetConnection.id, {
                    type: "take_snapshot",
    ...
    server.tool("type", "Type text into an editable element", {
        browserId: z
            .string()
            .optional()
            .describe("Browser connection ID (uses first available if not specified)"),
        ...
        captureSnapshot: z
            ...
            .default(true)
            .describe("Capture accessibility snapshot after action"),
    }, async ({ browserId, element, ref, text, slowly, submit, captureSnapshot, }) => {
        try {
            const connections = browserConnectionManager.getConnections();
            if (connections.length === 0) {
                return {
    ...
    // Tool: Press a key
    server.tool("press_key", "Press a key on the keyboard", {
        browserId: z
            .string()
            .optional()
            .describe("Browser connection ID (uses first available if not specified)"),
        ...
        captureSnapshot: z
            ...
            .default(true)
            .describe("Capture accessibility snapshot after action"),
    }, async ({ browserId, key, captureSnapshot }) => {
        try {
            const connections = browserConnectionManager.getConnections();
            if (connections.length === 0) {
                return {
    ...
    server.tool("select_option", "Select an option in a dropdown", {
        browserId: z
            .string()
            .optional()
            .describe("Browser connection ID (uses first available if not specified)"),
        ...
        captureSnapshot: z
            ...
            .default(true)
            .describe("Capture accessibility snapshot after action"),
    }, async ({ browserId, element, ref, values, captureSnapshot, }) => {
        try {
            const connections = browserConnectionManager.getConnections();
            if (connections.length === 0) {
                return {
    ...
    server.tool("wait_for", "Wait for text to appear or disappear or a specified time to pass", {
        browserId: z
            .string()
            .optional()
            .describe("Browser connection ID (uses first available if not specified)"),
        ...
        captureSnapshot: z
            ...
            .default(true)
            .describe("Capture accessibility snapshot after action"),
    }, async ({ browserId, text, textGone, time, captureSnapshot, }) => {
        try {
            const connections = browserConnectionManager.getConnections();
            if (connections.length === 0) {
                return {
