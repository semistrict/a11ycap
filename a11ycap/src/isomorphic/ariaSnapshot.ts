...
interface YamlNodeWithRange {
  range?: [number, number] | yamlTypes.Range;
}

interface YamlKeyWithRange {
  range?: [number, number] | yamlTypes.Range;
}

interface YamlValueWithRange {
  range?: [number, number] | yamlTypes.Range;
}
...
export function parseAriaSnapshot(
  ...
  const convertSeq = (
    container: AriaTemplateRoleNode,
    seq: yamlTypes.YAMLSeq
  ) => {
    for (const item of seq.items) {
      ...
      errors.push({
        ...
        range: convertRange(
          (item as YamlNodeWithRange).range ?? seq.range ?? [0, 0]
        ),
      });
    }
  };
    ...
    for (const entry of map.items) {
      ...
      if (!keyIsString) {
        errors.push({
          ...
          range: convertRange(
            (entry.key as YamlKeyWithRange).range ?? map.range ?? [0, 0]
          ),
        });
        continue;
      }

      const key: yamlTypes.Scalar<string> =
        entry.key as yamlTypes.Scalar<string>;
      ...
      if (key.value === 'text') {
        ...
        if (!valueIsString) {
          errors.push({
            ...
            range: convertRange(
              (entry.value as YamlValueWithRange).range ?? map.range ?? [0, 0]
            ),
          });
          continue;
        }
...
export class KeyParser {
    ...
    try {
      ...
      if (e instanceof ParserError) {
        ...
        errors.push({
          message,
          range: [
            options.lineCounter?.linePos(text.range?.[0] ?? 0) ?? {
              line: 0,
              col: 0,
            },
            options.lineCounter?.linePos((text.range?.[0] ?? 0) + e.pos) ?? {
              line: 0,
              col: 0,
            },
          ],
  ...
  private _throwError(message: string, offset = 0): never {
