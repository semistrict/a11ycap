...
function escapeRegexForSelector(re: RegExp): string {
  ...
  if (re.unicode || (re as RegExp & { unicodeSets?: boolean }).unicodeSets)
    return String(re);
...
export function escapeHTMLAttribute(s: string): string {
  return s.replace(
    /[&<>"']/gu,
    (char) => escaped[char as keyof typeof escaped]
  );
}
export function escapeHTML(s: string): string {
  return s.replace(/[&<]/gu, (char) => escaped[char as keyof typeof escaped]);
}
