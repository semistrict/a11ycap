...
function computeAriaRef(ariaNode: AriaNode, options: InternalOptions) {
  ...
  interface ElementWithAriaRef extends Element {
    _ariaRef?: AriaRef;
  }

  let ariaRef: AriaRef | undefined;
  ariaRef = (ariaNode.element as ElementWithAriaRef)._ariaRef;
  if (
    !ariaRef ||
    ariaRef.role !== ariaNode.role ||
    ariaRef.name !== ariaNode.name
  ) {
    ariaRef = {
      role: ariaNode.role,
      name: ariaNode.name,
    ...
    (ariaNode.element as ElementWithAriaRef)._ariaRef = ariaRef;
  }
...
function toAriaNode(
  ...
  // Extract React information if enabled
  if (options.enableReact) {
    result.react = extractReactInfo(element) || undefined;
  }
...
export function matchesExpectAriaTemplate(
  rootElement: Element,
...
export function getAllElementsMatchingExpectAriaTemplate(
  rootElement: Element,
  template: AriaTemplateNode
): Element[] {
...
export function renderAriaTree(
  ...
  const options = toInternalOptions(publicOptions);
  const lines: string[] = [];
  const includeText = options.renderStringsAsRegex
    ? textContributesInfo
    : () => true;
  const renderString = options.renderStringsAsRegex
    ? convertToBestGuessRegex
    : (str: string) => str;
  const visit = (
    ...
    if (ariaNode.react) {
      if (ariaNode.react.componentName) {
        key += ` [component=${ariaNode.react.componentName}]`;
      }
      if (ariaNode.react.debugSource) {
        key += ` [source=${ariaNode.react.debugSource}]`;
      }
