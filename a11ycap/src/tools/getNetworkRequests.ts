...
async function executeGetNetworkRequests(message: GetNetworkRequestsMessage): Promise<any> {
  ...
  try {
    ...
    let networkRequests = entries
      ...
      .map(entry => ({
        ...
        ...(('encodedBodySize' in entry) && { encodedBodySize: (entry as any).encodedBodySize }),
        ...(('decodedBodySize' in entry) && { decodedBodySize: (entry as any).decodedBodySize }),
        ...(('responseStart' in entry) && { responseStart: Math.round((entry as any).responseStart) }),
        ...(('responseEnd' in entry) && { responseEnd: Math.round((entry as any).responseEnd) }),
      }));
