 ...
 */
function findFiberForDom(
  dom: Element
): { componentFiber: any; domFiber: any } | null {
  let node: any = dom;
  while (node) {
...
function getInteractionHints(componentFiber: any, domFiber?: any): string[] {
  ...
  const domProps = domFiber?.memoizedProps || {};
...
export function extractReactInfo(element: Element): ReactInfo | null {
  ...
  try {
    ...
    if (
      ...
      Object.keys(relevantProps).length > 0 ||
      debugSource
    ) {
      const result = {
        componentName,
        componentState,
        interactionHints:
          interactionHints.length > 0 ? interactionHints : undefined,
        relevantProps:
          Object.keys(relevantProps).length > 0 ? relevantProps : undefined,
        debugSource,
      };
      return result;
    }
