...
function getExplicitAriaRole(element: Element): AriaRole | null {
  ...
  const roles = (element.getAttribute('role') || '')
    ...
    .map((role) => role.trim());
  return (
    (roles.find((role) => validRoles.includes(role as AriaRole)) as AriaRole) ||
    null
  );
}
...
function belongsToDisplayNoneOrAriaHiddenOrNonSlottedOrInert(
  element: Element
): boolean {
...
function belongsToDisabledOptGroup(element: Element): boolean {
  return (
    elementSafeTagName(element) === 'OPTION' &&
...
export function receivesPointerEvents(element: Element): boolean {
  ...
  if (!cache) {
    throw new Error(
      'Aria caches not initialized. Call beginAriaCaches() first.'
    );
  }
